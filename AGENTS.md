...
All routes are pre-rendered as individual HTML files — `public/_redirects` proxying rules (200) serve directory `index.html` without redirecting, so `/blog/slug` serves cleanly without redirect. Unknown paths get a Cloudflare 404 (no SPA fallback; the site is fully SSG).
